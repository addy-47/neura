...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    // Animate the sphere with the latest message if available
    if (messages.length > 0 && window.echoSphereController) {
      const latestMessage = messages[messages.length - 1];
      window.echoSphereController.animateText(latestMessage.text);
    }
  }, [messages]);
        ...
        <div className="flex flex-col h-[calc(100vh-4rem)] relative">
          ...
          <div className="flex-grow w-full">
            <div className="w-full h-full rounded-xl overflow-hidden" style={{ minHeight: '50vh' }}>
              <EchoSphere 
                initialText={messages.length > 0 ? messages[messages.length - 1].text : "Hello! I'm Neura, your digital doppelganger."} 
                showUI={false}
              />
            </div>
          </div>
